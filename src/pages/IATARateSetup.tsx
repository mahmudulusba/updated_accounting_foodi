...
const mockRates: IATARate[] = [
  { id: '1', effectiveDate: '2026-02-01', currency: 'BDT', iataRate: 109.25, status: 'active', updatedBy: 'Admin', updatedAt: '2026-02-01' },
  { id: '2', effectiveDate: '2026-01-15', currency: 'BDT', iataRate: 108.75, status: 'active', updatedBy: 'Admin', updatedAt: '2026-01-15' },
  { id: '3', effectiveDate: '2026-02-01', currency: 'SGD', iataRate: 0.74, status: 'active', updatedBy: 'Admin', updatedAt: '2026-02-01' },
  { id: '4', effectiveDate: '2026-02-01', currency: 'INR', iataRate: 83.20, status: 'active', updatedBy: 'Admin', updatedAt: '2026-02-01' },
  { id: '5', effectiveDate: '2025-12-01', currency: 'BDT', iataRate: 107.50, status: 'inactive', updatedBy: 'Admin', updatedAt: '2025-12-01' },
];

export default function IATARateSetup() {
  const [rates, setRates] = useState<IATARate[]>(mockRates);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState<'all' | 'active' | 'inactive'>('all');
  const [showDialog, setShowDialog] = useState(false);
  const [editingRate, setEditingRate] = useState<IATARate | null>(null);
  const [pageSize, setPageSize] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);
  const [formData, setFormData] = useState({ effectiveDate: format(new Date(), 'yyyy-MM-dd'), currency: 'BDT', iataRate: '' });
  ...
  const openAddDialog = () => { setEditingRate(null); setFormData({ effectiveDate: format(new Date(), 'yyyy-MM-dd'), currency: 'BDT', iataRate: '' }); setShowDialog(true); };
  const openEditDialog = (rate: IATARate) => { setEditingRate(rate); setFormData({ effectiveDate: rate.effectiveDate, currency: rate.currency, iataRate: rate.iataRate.toString() }); setShowDialog(true); };

  const handleSave = () => {
    if (!formData.iataRate) { toast.error('Please fill in all required fields'); return; }
    if (editingRate) {
      setRates(rates.map(r => r.id === editingRate.id ? { ...r, effectiveDate: formData.effectiveDate, currency: formData.currency, iataRate: parseFloat(formData.iataRate), updatedAt: format(new Date(), 'yyyy-MM-dd') } : r));
      toast.success('IATA rate updated successfully');
    } else {
      setRates([{ id: Date.now().toString(), effectiveDate: formData.effectiveDate, currency: formData.currency, iataRate: parseFloat(formData.iataRate), status: 'active', updatedBy: 'Admin', updatedAt: format(new Date(), 'yyyy-MM-dd') }, ...rates]);
      toast.success('IATA rate added successfully');
    }
    setShowDialog(false);
  };
    ...
    <DashboardLayout>
      <div className="space-y-4">
        <nav className="text-sm text-muted-foreground">
          <span className="text-primary font-medium">Configuration</span> / IATA Rate Setup
        </nav>
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold">IATA Rate Setup</h1>
          <Button onClick={openAddDialog} className="gap-2"><Plus size={16} /> Add Rate</Button>
        </div>
        <div className="bg-primary/5 border border-primary/20 rounded-lg p-4 text-sm">
          <p className="font-semibold text-primary">IATA Rate</p>
          <p>Applied for all IATA-related transactions and payments.</p>
          ...
          <TableWithSearch>
            <TableHeader>
              <TableRow className="bg-table-header hover:bg-table-header">
                ...
                <TableHead className="text-table-header-foreground font-semibold text-right">IATA Rate (1 USD =)</TableHead>
            ...
            <TableBody>
              {paginatedRates.map((rate, idx) => (
                <TableRow key={rate.id}>
                  ...
                  <TableCell className="text-right font-mono">{rate.iataRate.toFixed(2)}</TableCell>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>{editingRate ? 'Edit IATA Rate' : 'Add New IATA Rate'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <Label className="text-xs font-semibold">IATA Rate (1 USD = {formData.currency}) <span className="text-destructive">*</span></Label>
              <Input type="number" step="0.01" placeholder="e.g., 109.25" value={formData.iataRate} onChange={(e) => setFormData({ ...formData, iataRate: e.target.value })} />
              <p className="text-xs text-muted-foreground">Used for IATA-related transactions only</p>
            </div>
          </div>
          <DialogFooter>
